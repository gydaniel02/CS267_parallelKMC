...
# Processor_filename = f"Processor_{int(4*(args.size**3))}_O.pickle"
...
# Run KMC
Processor_filename = f"Processor_{int(args.size)}_O.pickle"
RT_CMC_results_file = f"Delithiated_RT_DRX_{int(args.size)}_O.pickle"
EVO_file = f"Evolution_300K_{int(args.size)}_O.pickle"
# RT_CMC_results_file:str = "Delithiated_RT_DRX.pickle")

start_time = time.time()
kmc = Multi_Time_Scale_KMC(args.T_KMC, args.traj_steps, Processor_filename, EVO_file, RT_CMC_results_file)
#TODO: still need delithiated_RT which is a default arg
kmc.run_KMC()
duration = time.time() - start_time
