...
#This handles ray profiling as well
# @ray.remote(
# num_gpus=1, 
# runtime_env={ "nsight": {
#     "t": "cuda,cudnn,cublas",
...
# save_to_file = "my_task.prof"

"""Goals:
1) get processor to be read in on remote nodes. how: turn processor passes into processor_file and IO
2) clean up my main code"""
...
from fast_processes_Sydneydraft2 import Fast_Processes_MC #lithium shuffling
# from fast_processes import Fast_Processes_MC #lithium shuffling
...
# @profiled(name="Tet_Oct_Updater", print_stats=False, save_to_file=save_to_file)
...
# @profiled(name="fill_species_list_partial", print_stats=False, save_to_file=save_to_file)
...
# @profiled(name="GetHops", print_stats=False, save_to_file=save_to_file)
def GetHops(TM, nndict, Species_Lists, Mobile_Mntet, Mobile_MnOct, site_encodings, occ, barrier_map, processor_file):
...
def inspect_argument_types(**kwargs):
    for arg_name, value in kwargs.items():
        print(f"\nArgument: {arg_name}")
        print(f"  Outer type: {type(value)}")
        
        if isinstance(value, dict):
            inner_types = {type(k): type(v) for k, v in value.items()}
            print("  Dict inner types (sampled):")
            for k_type, v_type in list(inner_types.items())[:5]:
                print(f"    Key: {k_type}, Value: {v_type}")

        elif isinstance(value, list):
            inner_types = {type(v) for v in value}
            print("  List inner types (sampled):")
            for t in list(inner_types)[:5]:
                print(f"    Element type: {t}")

        elif isinstance(value, np.ndarray):
            print(f"  Numpy dtype: {value.dtype}")
            print(f"  Read-only: {not value.flags.writeable}")
            if value.size > 0:
                print(f"  Sample element type: {type(value.flat[0])}")

        else:
            print("  Not a container or array — no inner types.")

# # Example call:
# inspect_argument_types(
#     TM="Mn",
#     nns={"site1": [1, 2], "site2": [3, 4]},
#     Species_Lists={"Mn": ["Mn3+", "Mn4+"]},
#     Mobile_Mntet=np.array([[0.0, 0.1, 0.2]]),
#     Mobile_MnOct=np.array([[0.3, 0.4, 0.5]]),
#     site_encodings={"site1": 1, "site2": 0},
#     occ={"site1": np.array([0.5, 0.5])},
#     barrier_map={"hop": 0.4},
#     processor="MyProcessorObject"
# )
...
def main(num_cpus: int, T_KMC: int, traj_steps: int, processor_file: str, EVOFILE: str, RT_CMC_results_file:str = "Delithiated_RT_DRX.pickle"):
    ...
    """
    print("here")
    ray.init(logging_level="debug")  # Or "info"
    ...
    #why reset this??
    for species in ["Mn3", "Mn2", "Mn4", "Vac", "Ti4"]:
        if species not in Species_Lists.keys():
            Species_Lists[species] = []
        for cn in ["tet", "oct"]:
            Species_Lists[f"{species}_{cn}"] = []
    # Species_Lists["Mn3_tet"] = []
    # Species_Lists["Mn2_tet"] = []
    # Species_Lists["Mn3_oct"] = []
    # Species_Lists["Mn2_oct"] = []
    # Species_Lists["Vac_tet"]
    # Species_Lists["Mn2"]=[]
    print("\n\n\n Species Lists after initializing")
    print(Species_Lists.keys())
    print(Species_Lists.values())
    ...
    # @profiled(name="Tet_Oct_Updater2", print_stats=False, save_to_file=save_to_file)
    def Tet_Oct_Updater2(Species_Lists):
        ...
        for label, future_list in futures.items():
            ...
            print(result)
            if result:
                Species_Lists[label].append(result)
                # print("INSIDE RESULT")
    Tet_Oct_Updater2(Species_Lists)

    # Mn3list = Species_Lists["Mn3"]
    # Mn2list = Species_Lists["Mn2"]
    # Vaclist = Species_Lists["Vac"]
    # print(f"Mn3list {Mn3list}")
    ...
    remote_fill_species_list_partial = ray.remote(num_cpus=num_cpus)(fill_species_list_partial)

    # @profiled(name="Occupancy_Resetter", print_stats=False, save_to_file=save_to_file)
    def Occupancy_Resetter(Species_Lists):
        unwanted_lists = {'Li_Vac', "Mn3_tet", "Mn3_oct", "Mn2_tet", "Mn2_oct"}
        # Launch Ray tasks
        futures = []
        # print(f"Species List: {Species_Lists}, Species_Type_Map {species_type_map}")
        for species, value in species_type_map.items():
            if species in Species_Lists and species not in unwanted_lists:
                futures.append(remote_fill_species_list_partial.remote(Species_Lists[species], value, n_sites))
            # else: 
            #     futures.append(remote_fill_species_list_partial.remote(Species_Lists[species], value, n_sites))
        ...
        print(species_list)
        #does this mutate and do I need it other than for occupancy

        return processor.encode_occupancy(species_list)
    ...
    # energy = 0 #start energy
    av_energy = 0
    Energy_All = np.array([])
    
    T_KMC = T_KMC
    e_cut = 6.96*10**-3 * T_KMC #TODO: what is this used for??
    T_sample = 2000 # for fast processes
    ...
    for s in range(traj_steps):    
        print(f"Step {s}")
        # 'T_sample', 'Tet_Oct_Updater', 'Occupancy_Resetter', 'occ', 'e_cut', 'kB', 'T_KMC', 'indices', and 's'

        Species_Lists, occ, energy = Fast_Processes.Select_Fast_Configuration(energy, Energy_All, Species_Lists, T_sample, Tet_Oct_Updater2, Occupancy_Resetter, occ, e_cut, kB, T_KMC, indices, Li, Vac, Mn2, Mn3, Mn4, site_encodings, processor_file, s)      #JUST TO PROVE THAT WE HAVE DONE OUR DUE DILLIGENCE
        print(f"Step {s} done with FAST PROCESSES")
        ...
        future_heavyhops = [remote_HeavyGetHops.remote(TM, nns, Species_Lists, Mobile_Mntet, Mobile_MnOct, site_encodings, occ, barrier_map, processor_file) for TM in Mobile_Mn2_tet + Mobile_Mn2_oct + Mobile_Mn3_tet + Mobile_Mn3_oct]
        future_lighthops = [remote_LightGetHops.remote(TM, nns, Species_Lists, Mobile_Mntet, Mobile_MnOct, site_encodings, occ, barrier_map, processor_file) for TM in Mn4_oct + Ti4_oct]
        ...
        inspect_argument_types(Mobile_Mn2_tet, nns, Species_Lists, Mobile_Mntet, Mobile_MnOct, site_encodings, occ, barrier_map, processor)
        ...
        for future in both_futures:
            ...
            if result["counter"]>0:
                ...
                All_Hops["Hops"]
        print(f"Step {s} done with ALL_HOPS")
        ...
        #         'Encoding': encoding
        #     }
        print(f"Step {s} done with SELECTING HOP")
    ...
    print(f"Step {s} done with EXECUTING HOP")



if __name__ == "__main__":
    # Example values — adjust to your needs
    main(
        num_cpus=14,
        T_KMC=300,
        traj_steps=10,
        processor_file="Processor_256_O.pickle",
        EVOFILE="Evolution_300K.txt",
        RT_CMC_results_file="Delithiated_RT_DRX_256_O.pickle"
    )
